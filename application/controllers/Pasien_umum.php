...
class Pasien_umum extends MY_Controller {
    ...
    {
        parent::__construct();
        $this->load->model('Pasien_umum_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'pasien_umum/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'pasien_umum/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'pasien_umum/index.html';
            $config['first_url'] = base_url() . 'pasien_umum/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Pasien_umum_model->total_rows($q);
        $pasien_umum = $this->Pasien_umum_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'pasien_umum_data' => $pasien_umum,
            'q' => $q,
        ...
        $this->load->view('pasien_umum_list', $data);
        $this->load->view('footer');
    }

    public function read($id) 
    {
        $row = $this->Pasien_umum_model->get_by_id($id);
        if ($row) {
            $data = array(
		'id' => $row->id,
		...
		'tanggal_daftar' => $row->tanggal_daftar,
            ...
            $this->load->view('header');
            $this->load->view('pasien_umum_read', $data);
            $this->load->view('footer');
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('pasien_umum'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('pasien_umum/create_action'),
	    'id' => set_value('id'),
        ...
        $this->load->view('header');
        $this->load->view('pasien_umum_form', $data);
        $this->load->view('footer');
    }
    ...
    public function create_action() 
    {
		...
		'tanggal_daftar' => $this->input->post('tanggal_daftar',TRUE),
            ...
            $this->Pasien_umum_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('pasien_umum/update_action'),
    ...
    public function update_action() 
    {
		...
		'tanggal_daftar' => $this->input->post('tanggal_daftar',TRUE),
            ...
            $this->Pasien_umum_model->update($this->input->post('id', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Pasien_umum_model->delete($id);
	...
	foreach ($this->Pasien_umum_model->get_all() as $data) {
            $kolombody = 0;
    ...
    public function word()
    {
        ...
        header("Content-Disposition: attachment;Filename=pasien_umum.doc");

        $data = array(
            'pasien_umum_data' => $this->Pasien_umum_model->get_all(),
            'start' => 0
        );
        
        $this->load->view('pasien_umum_doc',$data);
    }

}

/* End of file Pasien_umum.php */
/* Location: ./application/controllers/Pasien_umum.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2020-04-13 09:10:00 */
/* http://harviacode.com */